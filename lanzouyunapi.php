...
// 硬编码的ID和密码
$hardcoded_id = "b0w8ldo0j";
$hardcoded_pw = "ruoshui";
...
function folder($data,$js) {
    ...
    do {
        ...
        foreach ($json["text"] as $v) {
            $allFiles[] = array(
                ...
                "size" => $v["size"],
                "time" => Text_conversion_time($v["time"]),
                "icon" => $v["p_ico"] ? "https://image.woozooo.com/image/ico/".$v["ico"]."?x-oss-process=image/auto-orient,1/resize,m_fill,w_100,h_100/format,png" : "https://assets.woozooo.com/assets/images/type/".$v["icon"].".gif",
            );
        }
    ...
    } while (count($json["text"]) == 50 && $page <= $maxPages); // 50是每页最大数量
    
    $info["list"] = $allFiles;
    $info["have_page"] = false; // 标记已获取所有页面
